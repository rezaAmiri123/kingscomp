...
func (s *MatchmakingTestSuite) TestMatchmaking_Join() {
	...
	for i := 0; i < config.Default.LobbyMaxPlayer-1; i++ {
		testJoin(int64(3 + i))
	}

	<-time.After(time.Millisecond * 500)

	assert.Equal(s.T(), int64(config.Default.LobbyMaxPlayer-1), zCount(s.T(), s.redisClient, "matchmaking"))
...
func (s *MatchmakingTestSuite) TestMatchmaking_JoinWithManyLobbies() {
	...
	st := time.Now()
	for i := 0; i < config.Default.LobbyMaxPlayer*1000; i++ {
		testJoin(int64(i) + 1)
	}
	...
	for lobbyId, count := range counter.counter {
		...
		assert.NoError(s.T(), err)
		assert.Len(s.T(), lobby.Participants, config.Default.LobbyMaxPlayer)
		assert.Equal(s.T(), count, config.Default.LobbyMaxPlayer)
